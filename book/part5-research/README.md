...
**5.3 審查與否證**——`edge-strategy-reviewer` 給每份草稿判 `PASS`／`REVISE`／`REJECT`。`REVISE` 觸發修正、再送審，最多兩輪；兩輪後還過不了的不是直接淘汰，是降級成 `research_probe`——留一條線索，不留一個承諾。
