...
更值得留意的是六個 artifact 的去向。`closed_thesis_record` 與 `backtest_validation` 只在這條 workflow 裡打轉；其餘四個——`postmortem_findings`、`performance_coach_report`、`next_session_operating_rules`，連同最後的 `lessons_log_entry`——`downstream_hints` 全部指向同一個地方：`monthly-performance-review`。第一到三部的判斷大多在同一天內兌現；第四部產出的東西，要等到月初那個週末才會被再打開來看一次。
...
**4.4 模型書：把熟練度變成資料**——`stockbee-setup-fluency-trainer` 走的是另一條、不在這五步之列的迴圈。2.3 節篩出的候選，不管最後有沒有被 `technical-analyst` 接手、有沒有登記成論點，除了 `REJECTED` 一級都會被累積進模型書，3、5 個交易日後回頭補上結果。一次候選、一次記錄、一次覆核，單看不起眼；持續寫進同一本帳，熟練度才有機會變成看得見的資料。

實作章會把這五個步驟——包含可選的第 4 步讓 `backtest-expert` 回頭重驗原本的假說、必經的第 5 步由 `trader-memory-core` 把整輪檢討追加成 `lessons_log_entry`——接在某個星期三的平倉之後，實際跑一遍。
