...
三支 skill 的資料來源都以 FMP 為主，程度不一。`kanchi-dividend-sop` 只有 `build_entry_signals.py` 明講需要 `FMP_API_KEY`；`value-dividend-screener`、`dividend-growth-pullback-screener` 兩支輔助 screener 都支援 FINVIZ Elite 兩階段篩選——先用 FINVIZ 粗篩、FMP 再做細部分析，但省法不同：`value-dividend-screener` 省下 60–94% 的 FMP 呼叫量；`dividend-growth-pullback-screener` 沒有這個總量百分比，靠 FINVIZ 先收斂候選，執行時間從 10–15 分鐘壓到 2–3 分鐘。兩支都聲明 FINVIZ 是選配，FMP 才是硬性前提。這正是 `api_profile: mixed` 的來歷。
