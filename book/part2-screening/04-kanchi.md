...
兩支輔助 screener 不是 Kanchi SOP 的一部分，是餵候選給它的兩條不同管道，服務兩種不同的存股目標。`value-dividend-screener` 找的是「現在就要領息」的名字：`P/E` 低於 20、`P/B` 低於 2、殖利率 3% 以上——這是它兩階段篩選與 FMP-only 篩選步驟裡寫的數字；但同一份 SKILL.md 的報告範本、`screening_methodology.md` 的正式門檻表，跟腳本裡的實際預設參數（`dividend_yield_min=3.5`），殖利率門檻都寫成 3.5%——同一支 skill 裡兩個數字並存，讀哪一段文件，答案不一樣。外加三年期股息年複合成長率至少 5%、營收與 EPS 都要是正向趨勢。`dividend-growth-pullback-screener` 反過來找「現在殖利率不高、但成長快」的名字：三年股息年複合成長率至少 12%、起始殖利率門檻只要 1.5%，鎖定的是還在回檔的強勢股——`RSI` 40 以下。這兩種目標，相當貼近 `default-thresholds.md` 自己列出的三檔 profile 兩端：income now 殖利率下限 4.0%、growth first 只要 1.5–2.5%。
