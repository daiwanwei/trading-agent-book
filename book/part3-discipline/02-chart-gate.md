...
`decision_question` 只問一件事：候選裡誰有「乾淨的週線結構」，能通過人工複核。及格的結構只有三種：Stage 2 上升趨勢、tight base（緊縮的整理區），或是 Stockbee 式從受控底部展開的區間擴張。三選一，符合其中一種就算過關；一種都套不上，就不過。exhaustion hammer 候選還要多一道檢查——`decision_question` 與 `manual_review` 都寫明：要確認回檔不是由會摧毀論點的新聞事件引發，且風險到當日低點是可以承受的。這不是額外加碼的審查，而是因為 exhaustion hammer 這個型態本身就長在一次急拉之後的回檔裡，光看「有沒有 Stage 2 結構」不足以分辨它究竟是趨勢真的轉弱，還是健康的獲利了結。
