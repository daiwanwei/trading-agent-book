...
06:44，離開盤還有 46 分鐘。桌上躺著三份剛出爐的報告：`market_breadth_report`、`uptrend_report`、`top_risk_report`——1.1 到 1.3 節走過的三個 skill，各自留下的痕跡。三份報告量的是完全不同的東西：一份是今天到底有多少股票真的在漲，一份是上升趨勢的參與廣度，第三份（如果第 3 步沒被跳過）是頭部風險積累到什麼程度。三份都攤開了，卻沒有一份告訴 Agent 接下來該做什麼。
...
`exposure_framework.md` 把仲裁規則分成兩層。第一層是訊號大致同向時：八個分項（regime 25%、top risk 20%、breadth 15%、uptrend 15%、institutional 10%、sector 5%、theme 5%、ftd 5%）用固定權重合成一個 composite score，分歧被平均掉。第二層才是真正的仲裁——`Recommendation Logic` 底下，REDUCE_ONLY 與 CASH_PRIORITY 的判定條件都用「OR」串起來：composite score 落在特定區間，或 top risk score 落在特定區間，或關鍵輸入缺兩項以上，任一項成立就觸發，不必全部同時成立。也就是說，就算其他七項都健康，只要 top risk 這一項亮紅燈，composite 平均分數可能還撐在及格線之上，OR 邏輯照樣讓這一項單獨否決，把姿態往保守拉。`regime_exposure_map.md` 對訊號衝突的原則寫得更直接：預設採更保守的姿態、調降信心等級、把衝突寫進 rationale——不是取折衷，是靠向壞消息那一邊。
...
門關上之後，後果不是一句提醒，而是寫進機制裡的：`manual_review` 清單最後一條明講，`exposure_decision` 偏限制時，就把第二部的 `swing-opportunity-daily` 往後延。這是全書遇到的第一道 fail-closed 閘門——不是靠人記得謹慎，是靠流程本身在源頭把後面十步一次堵住。第三部會遇到同一種設計用在帳戶層級的斷路器上；姿態是第一道，帳戶是第二道，兩道閘門守的是不同的事——一個問市場准不准，一個問你的帳戶准不准。
